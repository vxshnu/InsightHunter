...
train = [
    ...
    ("How can I find rows where 'Price' is more than 50? (columns: Product_ID, Name, Price, Stock)", "df[df['Price'] > 50]"),
    ("Which rows have 'Category' equal to 'Electronics'? (columns: Product_ID, Name, Category, Price)", "df[df['Category'] == 'Electronics']"),
    ("Can I get all rows where 'Salary' is greater than 70000? (columns: Employee_ID, Name, Job_Title, Salary)", "df[df['Salary'] > 70000]"),
    ("Show me entries where 'City' is 'New York'. (columns: Customer_ID, Name, City, Purchases)", "df[df['City'] == 'New York']"),
    ("What are the rows where 'Stock' is less than 10? (columns: Product_ID, Name, Stock, Price)", "df[df['Stock'] < 10]"),
    ("Give me all flights going to 'London'. (columns: Flight_ID, Airline, Destination, Status)", "df[df['Destination'] == 'London']"),
    ("Which rows have 'Attendance' below 75%? (columns: Student_ID, Name, Attendance, Grade)", "df[df['Attendance'] < 75]"),
    ("How do I filter rows where 'Fuel_Type' is 'Electric'? (columns: Vehicle_ID, Brand, Fuel_Type, Range)", "df[df['Fuel_Type'] == 'Electric']"),
    ("I need all the rows where 'Score' is above 90. (columns: Student_ID, Name, Subject, Score)", "df[df['Score'] > 90]"),
    ("Can you show me products with 'Brand' as 'Sony'? (columns: Product_ID, Brand, Category, Price)", "df[df['Brand'] == 'Sony']"),
    ("How do I find all employees with more than 5 years of experience? (columns: Employee_ID, Name, Experience, Salary)", "df[df['Experience'] > 5]"),
    ("Get the data where 'Genre' is 'Comedy'. (columns: Movie_ID, Title, Genre, Rating)", "df[df['Genre'] == 'Comedy']"),
    ("What rows have 'Status' marked as 'Completed'? (columns: Task_ID, Task_Name, Status, Deadline)", "df[df['Status'] == 'Completed']"),
    ("How do I get rows where 'Grade' is 'A'? (columns: Student_ID, Name, Grade, Score)", "df[df['Grade'] == 'A']"),
    ("Can you find rows with 'Diagnosis' as 'Diabetes'? (columns: Patient_ID, Age, Diagnosis, Treatment)", "df[df['Diagnosis'] == 'Diabetes']"),
    ("Show me rows where 'Rating' is below 3. (columns: Product_ID, Product, Rating, Price)", "df[df['Rating'] < 3]"),
    ("Which entries have 'Profit_Margin' greater than 20%? (columns: Store_ID, Revenue, Expenses, Profit_Margin)", "df[df['Profit_Margin'] > 20]"),
    ("Get me all transactions with 'Amount' over 1000. (columns: Transaction_ID, Customer, Amount, Date)", "df[df['Amount'] > 1000]"),
    ("How can I list flights delayed for more than 30 minutes? (columns: Flight_ID, Airline, Delay, Status)", "df[df['Delay'] > 30]"),
    ("Find all rows where 'Project_Status' is 'Ongoing'. (columns: Project_ID, Name, Project_Status, Budget)", "df[df['Project_Status'] == 'Ongoing']"),
    ("Can I see rows where 'Medals' are greater than 3? (columns: Athlete_ID, Name, Sport, Medals)", "df[df['Medals'] > 3]"),
    ("How do I get products priced between 100 and 200? (columns: Product_ID, Name, Price, Stock)", "df[(df['Price'] >= 100) & (df['Price'] <= 200)]"),
    ("Which rows have 'City' listed as 'Tokyo'? (columns: Customer_ID, Name, City, Purchases)", "df[df['City'] == 'Tokyo']"),
    ("Can you show me students with 'Attendance' over 90%? (columns: Student_ID, Name, Attendance, Grade)", "df[df['Attendance'] > 90]"),
    ("I need to see rows where 'Role' is 'Manager'. (columns: Employee_ID, Name, Role, Salary)", "df[df['Role'] == 'Manager']"),
    ("How do I filter rows with 'Temperature' above 25? (columns: Date, City, Temperature, Humidity)", "df[df['Temperature'] > 25]"),
    ("What are the rows where 'Payment_Method' is 'Credit Card'? (columns: Transaction_ID, Customer, Payment_Method, Amount)", "df[df['Payment_Method'] == 'Credit Card']"),
    ("Can you give me rows where 'Revenue' is above 100000? (columns: Store_ID, Revenue, Expenses, Profit)", "df[df['Revenue'] > 100000]"),
    ("How do I get rows where 'Fuel_Efficiency' is more than 20 km/l? (columns: Vehicle_ID, Model, Fuel_Efficiency, Price)", "df[df['Fuel_Efficiency'] > 20]"),
    ("Find products with a 'Stock_Availability' of 'In Stock'. (columns: Product_ID, Product, Stock_Availability, Price)", "df[df['Stock_Availability'] == 'In Stock']"),
    ("Can I list rows where 'Genre' is 'Action'? (columns: Movie_ID, Title, Genre, Rating)", "df[df['Genre'] == 'Action']"),
    ("Which rows have 'Department' as 'Finance'? (columns: Employee_ID, Name, Department, Salary)", "df[df['Department'] == 'Finance']"),
    ("Show me all entries where 'Age' is greater than 60. (columns: Patient_ID, Name, Age, Diagnosis)", "df[df['Age'] > 60]"),
    ("How do I filter rows where 'Weather' is 'Rainy'? (columns: Date, City, Weather, Temperature)", "df[df['Weather'] == 'Rainy']"),
    ("Get rows with 'Amount_Spent' under 500. (columns: Customer_ID, Name, Amount_Spent, City)", "df[df['Amount_Spent'] < 500]"),
    ("Can you find rows where 'Season' is 'Winter'? (columns: Event_ID, Event_Name, Season, Participants)", "df[df['Season'] == 'Winter']"),
    ("How do I get rows where 'Test_Score' is above the average? (columns: Student_ID, Name, Test_Score, Subject)", "df[df['Test_Score'] > df['Test_Score'].mean()]"),
    ("Show all rows where 'Region' is 'South'. (columns: Store_ID, Name, Revenue, Region)", "df[df['Region'] == 'South']"),
    ("Which flights are going to 'Mumbai'? (columns: Flight_ID, Airline, Destination, Status)", "df[df['Destination'] == 'Mumbai']"),
    ("How do I find rows where 'Position' is 'Forward'? (columns: Player_ID, Name, Position, Goals)", "df[df['Position'] == 'Forward']"),
    ("Show rows where 'Attendance' is less than 50%. (columns: Student_ID, Name, Attendance, Grade)", "df[df['Attendance'] < 50]"),
    ("Can I get rows where 'Rating' is exactly 5? (columns: Book_ID, Title, Author, Rating)", "df[df['Rating'] == 5]"),
    ("How do I list employees who have worked more than 10 years? (columns: Employee_ID, Name, Years_Worked, Department)", "df[df['Years_Worked'] > 10]"),
    ("What rows have 'Order_Quantity' below 5? (columns: Order_ID, Product, Order_Quantity, Price)", "df[df['Order_Quantity'] < 5]"),
    ("Can you show rows where 'Age' is between 20 and 30? (columns: Person_ID, Name, Age, Gender)", "df[(df['Age'] >= 20) & (df['Age'] <= 30)]"),
    ("How do I get rows where 'Budget' exceeds 100000? (columns: Project_ID, Name, Budget, Duration)", "df[df['Budget'] > 100000]"),
    ("Find rows with 'Grade' not equal to 'F'. (columns: Student_ID, Name, Grade, Subject)", "df[df['Grade'] != 'F']"),
    ("Which rows have 'Course' as 'Data Science'? (columns: Student_ID, Name, Course, Grade)", "df[df['Course'] == 'Data Science']"),
    ("Get me all tasks that are overdue. (columns: Task_ID, Task_Name, Deadline, Status)", "df[df['Status'] == 'Overdue']"),
    ("How do I find rows where 'Country' is 'France'? (columns: Customer_ID, Name, Country, Purchases)", "df[df['Country'] == 'France']"),
    ("Show entries where 'Profit' is less than 0. (columns: Store_ID, Revenue, Expenses, Profit)", "df[df['Profit'] < 0]"),
    ("What rows have 'Attendance' exactly at 80%? (columns: Student_ID, Name, Attendance, Score)", "df[df['Attendance'] == 80]"),
    ("Find rows where 'Medals' are greater than or equal to 10. (columns: Athlete_ID, Name, Sport, Medals)", "df[df['Medals'] >= 10]"),
    ("How can I filter rows where 'Age' is greater than 30? (columns: ID, Name, Age, Salary)", "df[df['Age'] > 30]"),
    ("Which products have a 'Price' less than 50? (columns: Product_ID, Product_Name, Price, Stock)", "df[df['Price'] < 50]"),
    ("Can you show me all employees in the 'IT' department? (columns: Employee_ID, Name, Department, Salary)", "df[df['Department'] == 'IT']"),
    ("How do I find entries where 'City' is 'Berlin'? (columns: Customer_ID, Name, City, Orders)", "df[df['City'] == 'Berlin']"),
    ("Get rows where 'Grade' is 'A' or 'B'. (columns: Student_ID, Name, Grade, Subject)", "df[df['Grade'].isin(['A', 'B'])]"),
    ("Show all customers who made purchases over $1000. (columns: Customer_ID, Name, Purchase_Amount, City)", "df[df['Purchase_Amount'] > 1000]"),
    ("Can you give me data where 'Temperature' is below freezing? (columns: Date, City, Temperature, Humidity)", "df[df['Temperature'] < 0]"),
    ("What are the rows where 'Rating' is exactly 4.5? (columns: Product_ID, Product, Rating, Price)", "df[df['Rating'] == 4.5]"),
    ("How do I filter flights delayed by more than an hour? (columns: Flight_ID, Airline, Delay, Status)", "df[df['Delay'] > 60]"),
    ("Find tasks with 'Status' as 'Pending'. (columns: Task_ID, Task_Name, Status, Deadline)", "df[df['Status'] == 'Pending']"),
    ("Can you find entries where 'Revenue' exceeds $10,000? (columns: Store_ID, Name, Revenue, Profit)", "df[df['Revenue'] > 10000]"),
    ("How do I find rows where 'Attendance' is below 70%? (columns: Student_ID, Name, Attendance, Grade)", "df[df['Attendance'] < 70]"),
    ("List all entries with 'Payment_Method' as 'PayPal'. (columns: Transaction_ID, Customer, Payment_Method, Amount)", "df[df['Payment_Method'] == 'PayPal']"),
    ("What rows have 'Fuel_Efficiency' greater than 25 km/l? (columns: Vehicle_ID, Model, Fuel_Efficiency, Price)", "df[df['Fuel_Efficiency'] > 25]"),
    ("How can I get rows where 'Position' is 'Manager'? (columns: Employee_ID, Name, Position, Salary)", "df[df['Position'] == 'Manager']"),
    ("Show all students with grades higher than 80. (columns: Student_ID, Name, Grade, Score)", "df[df['Score'] > 80]"),
    ("Can you list rows where 'Order_Status' is 'Shipped'? (columns: Order_ID, Customer, Order_Status, Amount)", "df[df['Order_Status'] == 'Shipped']"),
    ("Which rows have 'Age' between 18 and 25? (columns: Person_ID, Name, Age, Gender)", "df[(df['Age'] >= 18) & (df['Age'] <= 25)]"),
    ("Get rows where 'Brand' is 'Apple'. (columns: Product_ID, Brand, Price, Stock)", "df[df['Brand'] == 'Apple']"),
    ("Find flights with 'Destination' as 'Paris'. (columns: Flight_ID, Airline, Destination, Status)", "df[df['Destination'] == 'Paris']"),
    ("Show rows where 'Region' is 'North'. (columns: Store_ID, Name, Revenue, Region)", "df[df['Region'] == 'North']"),
    ("Can you show me employees earning above $70,000? (columns: Employee_ID, Name, Salary, Department)", "df[df['Salary'] > 70000]"),
    ("Which tasks are due tomorrow? (columns: Task_ID, Task_Name, Deadline, Status)", "df[df['Deadline'] == 'tomorrow']"),
    ("How do I filter rows where 'Course' is 'Python Programming'? (columns: Student_ID, Name, Course, Grade)", "df[df['Course'] == 'Python Programming']"),
    ("Get rows with 'Diagnosis' as 'Hypertension'. (columns: Patient_ID, Age, Diagnosis, Treatment)", "df[df['Diagnosis'] == 'Hypertension']"),
    ("Can you list products priced under $30? (columns: Product_ID, Name, Price, Stock)", "df[df['Price'] < 30]"),
    ("Find rows where 'Attendance' is exactly 100%. (columns: Student_ID, Name, Attendance, Grade)", "df[df['Attendance'] == 100]"),
    ("What are the entries with 'Weather' as 'Rainy'? (columns: Date, City, Weather, Temperature)", "df[df['Weather'] == 'Rainy']"),
    ("Which players scored more than 20 points? (columns: Player_ID, Name, Team, Points)", "df[df['Points'] > 20]"),
    ("How do I get rows where 'Test_Score' is below 50? (columns: Student_ID, Name, Test_Score, Subject)", "df[df['Test_Score'] < 50]"),
    ("Show employees working in the 'HR' department. (columns: Employee_ID, Name, Department, Salary)", "df[df['Department'] == 'HR']"),
    ("Can you list flights that were cancelled? (columns: Flight_ID, Airline, Status, Delay)", "df[df['Status'] == 'Cancelled']"),
    ("Get products where 'Stock' is more than 100. (columns: Product_ID, Name, Stock, Price)", "df[df['Stock'] > 100]"),
    ("Find rows where 'Grade' is 'C'. (columns: Student_ID, Name, Grade, Subject)", "df[df['Grade'] == 'C']"),
    ("What rows have 'Rating' less than 2.0? (columns: Movie_ID, Title, Genre, Rating)", "df[df['Rating'] < 2.0]"),
    ("Show me customers from 'California'. (columns: Customer_ID, Name, State, Orders)", "df[df['State'] == 'California']"),
    ("Can you get all products in the 'Toys' category? (columns: Product_ID, Name, Category, Price)", "df[df['Category'] == 'Toys']"),
    ("Which orders were made on '2023-05-10'? (columns: Order_ID, Customer, Date, Amount)", "df[df['Date'] == '2023-05-10']"),
    ("How do I find entries where 'Profit' is negative? (columns: Store_ID, Revenue, Expenses, Profit)", "df[df['Profit'] < 0]"),
    ("Show rows where 'Attendance' is greater than the average. (columns: Student_ID, Name, Attendance, Grade)", "df[df['Attendance'] > df['Attendance'].mean()]"),
    ("What tasks are marked as 'Completed'? (columns: Task_ID, Task_Name, Status, Deadline)", "df[df['Status'] == 'Completed']"),
    ("Can you filter rows where 'Region' is 'East' and 'Sales' are above $5,000? (columns: Sales_ID, Region, Sales, Profit)", "df[(df['Region'] == 'East') & (df['Sales'] > 5000)]"),
    ("Get employees who joined before 2020. (columns: Employee_ID, Name, Join_Date, Department)", "df[df['Join_Date'] < '2020-01-01']"),
    ("Which rows have 'Medals' greater than or equal to 5? (columns: Athlete_ID, Name, Sport, Medals)", "df[df['Medals'] >= 5]"),
    ("Find all rows where 'Stock_Availability' is 'Out of Stock'. (columns: Product_ID, Product, Stock_Availability, Price)", "df[df['Stock_Availability'] == 'Out of Stock']"),
    ("Can you show rows where 'Fuel_Type' is 'Hybrid'? (columns: Vehicle_ID, Brand, Fuel_Type, Mileage)", "df[df['Fuel_Type'] == 'Hybrid']"),
    ("Get customers whose 'Age' is below 25. (columns: Customer_ID, Name, Age, Purchases)", "df[df['Age'] < 25]"),
    ("How do I find students with 'Test_Score' exactly 100? (columns: Student_ID, Name, Test_Score, Subject)", "df[df['Test_Score'] == 100]"),
    ("Show products with 'Rating' over 4.7. (columns: Product_ID, Name, Rating, Price)", "df[df['Rating'] > 4.7]"),
    ("Can you find rows where 'Expenses' are less than $2,000? (columns: Store_ID, Revenue, Expenses, Profit)", "df[df['Expenses'] < 2000]"),
    ("Which movies were released after 2020? (columns: Movie_ID, Title, Release_Year, Genre)", "df[df['Release_Year'] > 2020]"),
    ("Get rows with 'Role' as 'Team Lead'. (columns: Employee_ID, Name, Role, Department)", "df[df['Role'] == 'Team Lead']"),
    ("Can you show all entries where 'Temperature' is above 35 degrees? (columns: Date, City, Temperature, Humidity)", "df[df['Temperature'] > 35]"),
    ("How do I get all rows where 'Salary' is over $80,000? (columns: Employee_ID, Name, Salary, Department)", "df[df['Salary'] > 80000]"),
    ("Can you show me employees working in 'Marketing'? (columns: Employee_ID, Name, Department, Salary)", "df[df['Department'] == 'Marketing']"),
    ("What rows have 'Attendance' below 60%? (columns: Student_ID, Name, Attendance, Grade)", "df[df['Attendance'] < 60]"),
    ("How can I filter rows where 'City' is 'Chicago'? (columns: Customer_ID, Name, City, Purchases)", "df[df['City'] == 'Chicago']"),
    ("List all products where 'Price' is greater than $500. (columns: Product_ID, Name, Price, Category)", "df[df['Price'] > 500]"),
    ("Which tasks have a deadline of '2023-12-01'? (columns: Task_ID, Task_Name, Deadline, Status)", "df[df['Deadline'] == '2023-12-01']"),
    ("Can you find all rows where 'Temperature' is less than 15°C? (columns: Date, City, Temperature, Weather)", "df[df['Temperature'] < 15]"),
    ("What are the rows where 'Profit' is negative? (columns: Store_ID, Name, Revenue, Profit)", "df[df['Profit'] < 0]"),
    ("How do I list flights going to 'Toronto'? (columns: Flight_ID, Airline, Destination, Status)", "df[df['Destination'] == 'Toronto']"),
    ("Show me students with a grade of 'B'. (columns: Student_ID, Name, Grade, Subject)", "df[df['Grade'] == 'B']"),
    ("Can you filter rows where 'Rating' is exactly 4.0? (columns: Movie_ID, Title, Genre, Rating)", "df[df['Rating'] == 4.0]"),
    ("Which rows have 'Stock' less than 50 units? (columns: Product_ID, Name, Stock, Price)", "df[df['Stock'] < 50]"),
    ("Find all athletes with more than 2 medals. (columns: Athlete_ID, Name, Sport, Medals)", "df[df['Medals'] > 2]"),
    ("How do I get rows where 'Fuel_Type' is 'Diesel'? (columns: Vehicle_ID, Brand, Fuel_Type, Mileage)", "df[df['Fuel_Type'] == 'Diesel']"),
    ("Show all orders placed on '2024-01-15'. (columns: Order_ID, Customer, Date, Amount)", "df[df['Date'] == '2024-01-15']"),
    ("Can you find employees who have been with the company for over 10 years? (columns: Employee_ID, Name, Years_At_Company, Salary)", "df[df['Years_At_Company'] > 10]"),
    ("What rows have 'Attendance' at exactly 75%? (columns: Student_ID, Name, Attendance, Grade)", "df[df['Attendance'] == 75]"),
    ("How can I list all rows where 'Country' is 'Japan'? (columns: Customer_ID, Name, Country, Orders)", "df[df['Country'] == 'Japan']"),
    ("Find rows where 'Sales' exceed $10,000. (columns: Sales_ID, Product, Sales, Region)", "df[df['Sales'] > 10000]"),
    ("Can you show entries where 'Payment_Method' is 'Cash'? (columns: Transaction_ID, Customer, Payment_Method, Amount)", "df[df['Payment_Method'] == 'Cash']"),
    ("Which products have a 'Category' of 'Appliances'? (columns: Product_ID, Name, Category, Price)", "df[df['Category'] == 'Appliances']"),
    ("How do I filter rows where 'Diagnosis' is 'Asthma'? (columns: Patient_ID, Name, Age, Diagnosis)", "df[df['Diagnosis'] == 'Asthma']"),
    ("Show flights delayed by more than 90 minutes. (columns: Flight_ID, Airline, Delay, Status)", "df[df['Delay'] > 90]"),
    ("Can you list rows where 'Grade' is either 'A' or 'B'? (columns: Student_ID, Name, Grade, Subject)", "df[df['Grade'].isin(['A', 'B'])]"),
    ("What are the rows with 'Region' as 'West'? (columns: Store_ID, Name, Region, Revenue)", "df[df['Region'] == 'West']"),
    ("Find tasks marked as 'In Progress'. (columns: Task_ID, Task_Name, Status, Deadline)", "df[df['Status'] == 'In Progress']"),
    ("How can I show all movies released after 2015? (columns: Movie_ID, Title, Release_Year, Genre)", "df[df['Release_Year'] > 2015]"),
    ("List employees earning less than $50,000. (columns: Employee_ID, Name, Salary, Department)", "df[df['Salary'] < 50000]"),
    ("Which orders are still 'Pending'? (columns: Order_ID, Customer, Status, Amount)", "df[df['Status'] == 'Pending']"),
    ("Can you find rows where 'Test_Score' is above 85? (columns: Student_ID, Name, Test_Score, Subject)", "df[df['Test_Score'] > 85]"),
    ("Get all rows where 'Temperature' is above 30°C. (columns: Date, City, Temperature, Humidity)", "df[df['Temperature'] > 30]"),
    ("How do I filter products with 'Stock_Availability' as 'In Stock'? (columns: Product_ID, Name, Stock_Availability, Price)", "df[df['Stock_Availability'] == 'In Stock']"),
    ("Show me entries where 'Fuel_Efficiency' is below 15 km/l. (columns: Vehicle_ID, Brand, Fuel_Efficiency, Price)", "df[df['Fuel_Efficiency'] < 15]"),
    ("What rows have 'City' listed as 'San Francisco'? (columns: Customer_ID, Name, City, Purchases)", "df[df['City'] == 'San Francisco']"),
    ("Can you list students with grades lower than 60? (columns: Student_ID, Name, Grade, Score)", "df[df['Score'] < 60]"),
    ("How do I find rows where 'Position' is 'Team Lead'? (columns: Employee_ID, Name, Position, Department)", "df[df['Position'] == 'Team Lead']"),
    ("Find tasks due by '2024-03-01'. (columns: Task_ID, Task_Name, Deadline, Status)", "df[df['Deadline'] <= '2024-03-01']"),
    ("Can you show rows where 'Medals' are equal to 0? (columns: Athlete_ID, Name, Sport, Medals)", "df[df['Medals'] == 0]"),
    ("Which flights were cancelled due to weather? (columns: Flight_ID, Airline, Cancellation_Reason, Status)", "df[df['Cancellation_Reason'] == 'Weather']"),
    ("List products where 'Price' is under $20. (columns: Product_ID, Name, Price, Stock)", "df[df['Price'] < 20]"),
    ("How can I filter rows where 'Revenue' exceeds $50,000? (columns: Store_ID, Name, Revenue, Expenses)", "df[df['Revenue'] > 50000]"),
    ("Find rows where 'Diagnosis' is 'Heart Disease'. (columns: Patient_ID, Name, Age, Diagnosis)", "df[df['Diagnosis'] == 'Heart Disease']"),
    ("What are the rows where 'Stock' is greater than 500? (columns: Product_ID, Name, Stock, Price)", "df[df['Stock'] > 500]"),
    ("Show flights departing from 'New York'. (columns: Flight_ID, Airline, Origin, Destination)", "df[df['Origin'] == 'New York']"),
    ("Which students have an 'Attendance' above average? (columns: Student_ID, Name, Attendance, Grade)", "df[df['Attendance'] > df['Attendance'].mean()]"),
    ("Can you show employees with a 'Role' of 'Software Engineer'? (columns: Employee_ID, Name, Role, Salary)", "df[df['Role'] == 'Software Engineer']"),
    ("How do I find rows where 'Order_Quantity' is less than 10? (columns: Order_ID, Product, Order_Quantity, Amount)", "df[df['Order_Quantity'] < 10]"),
    ("What rows have 'Test_Score' below 50? (columns: Student_ID, Name, Test_Score, Subject)", "df[df['Test_Score'] < 50]"),
    ("Show customers from 'India'. (columns: Customer_ID, Name, Country, Orders)", "df[df['Country'] == 'India']"),
    ("Which tasks are overdue? (columns: Task_ID, Task_Name, Status, Deadline)", "df[df['Status'] == 'Overdue']"),
    ("Can you list all entries where 'Payment_Method' is 'Bank Transfer'? (columns: Transaction_ID, Customer, Payment_Method, Amount)", "df[df['Payment_Method'] == 'Bank Transfer']"),
    ("What are the rows where 'Grade' is not 'F'? (columns: Student_ID, Name, Grade, Subject)", "df[df['Grade'] != 'F']"),
    ("How do I filter rows where 'Fuel_Type' is 'Hybrid'? (columns: Vehicle_ID, Model, Fuel_Type, Mileage)", "df[df['Fuel_Type'] == 'Hybrid']"),
    ("Find rows where 'Stock_Availability' is 'Out of Stock'. (columns: Product_ID, Name, Stock_Availability, Price)", "df[df['Stock_Availability'] == 'Out of Stock']"),
    ("Which rows have 'Profit_Margin' greater than 25%? (columns: Store_ID, Revenue, Expenses, Profit_Margin)", "df[df['Profit_Margin'] > 25]"),
    ("Can you show movies rated above 4.8? (columns: Movie_ID, Title, Genre, Rating)", "df[df['Rating'] > 4.8]")
]
...
test =[
    ...
    ("How do I filter rows where 'Temperature' is over 35°C? (columns: Date, City, Temperature, Humidity)", "df[df['Temperature'] > 35]"),
